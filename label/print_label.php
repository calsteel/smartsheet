...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['image_data']) && isset($_POST['num_copies']) && isset($_POST['printer'])) {
        $image_data = base64_decode($_POST['image_data']);
        $zpl_code = convertToZPL($image_data);
        $num_copies = intval($_POST['num_copies']);
        ...
        if ($zpl_code) {
            for ($i = 0; $i < $num_copies; $i++) {
                ...
                if ($return_var !== 0) {
                    echo "Failed to print copy " . ($i + 1) . ". Return code: $return_var\n";
                    echo "Output:\n" . implode("\n", $output);
                    return;
                }
            }
        ...
        echo "Error: Image data or number of copies not received.";
    }
} else {
